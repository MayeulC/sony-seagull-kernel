...
int alog_ram_console_write_log(const LogType log_type, const void __user *user_buf, char *kernel_buf, int count){  //FIH-SW3-KERNEL-BH-last_alog-01*
	...
	if (overrun)
	{
	  	...
	  	if (user_buf)
	  	{
			if (copy_from_user(buffer->data + buffer->start, (const void __user *)orig_s, rem))
			{
				memset(buffer->data + buffer->start,0,rem);		
	...
	if (user_buf)
	{
		if (copy_from_user(buffer->data + buffer->start, (const void __user *)s, count))
		{
			memset(buffer->data + buffer->start,0,count);			
			pr_err("[LastAlog] copy from user failed!\n");
		}
	}
	else
