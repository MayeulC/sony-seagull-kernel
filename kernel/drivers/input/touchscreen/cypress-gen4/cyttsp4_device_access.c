...
#include <linux/input/cypress-gen4/cyttsp4_mt.h>
...
static DEVICE_ATTR(raw_mutual_limit, S_IRUSR | S_IWUSR,
...
int  cyttsp4_bist_raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int rc = 0;
	char * grpnum_test = "15";
	char * grpoffset = "0";
...
int  cyttsp4_bist_auto_short_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int rc = 0;
	char * grpnum_test = "13";
	char * grpoffset = "0";
...
int  cyttsp4_bist_open_show(struct device *dev, struct device_attribute *attr, char *buf)
{
     int rc = 0;
     char * grpnum_test = "13";
     char * grpoffset = "0";
...
static DEVICE_ATTR(bist_open, S_IRUSR | S_IWUSR,
...
static ssize_t cyttsp4_bist_idac_check(struct device *dev, char *buf, int print_idx, enum cyttsp4_idac_type type)
{
	...
	print_idx += scnprintf(buf + print_idx, CY_MAX_PRBUF_SIZE, "(%d bytes)\n", num_read);
...
static ssize_t cyttsp4_idac_print(struct device *dev, char *buf, int print_idx, enum cyttsp4_idac_type type)
{
	...
	print_idx += scnprintf(buf + print_idx, CY_MAX_PRBUF_SIZE, "(%d data)\n", num_read - 6);
...
int  cyttsp4_raw_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int rc = 0;
	char * grpnum_test = "15";
	char * grpoffset = "0";
