...
static int wpa_supplicant_select_config(struct wpa_supplicant *wpa_s)
{
	...
	if (ssid->disabled_for_auto_connect) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Selected network is not configured "
					 ...
					 struct wpa_bss *bss)
{
	...
	int proto_match = 0;
#ifdef CONFIG_WAPI
	const u8 *rsn_ie, *wpa_ie, *wapi_ie;
#else
					    ...
					    struct wpa_ssid *group)
{
	...
	wpa_printf(MSG_DEBUG, "%d: " MACSTR " ssid='%s' "
			"wpa_ie_len=%u rsn_ie_len=%u wapi_ie_len=%u caps=0x%x",
			(int) i, MAC2STR(bss->bssid),
			wpa_ssid_txt(bss->ssid,bss->ssid_len),
			wpa_ie_len, rsn_ie_len, wapi_ie_len, bss->caps);
#endif
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
		...
		if (ssid->disabled_for_auto_connect) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - no auto connect");
			continue;
		}
						 ...
						 int locally_generated)
{
	...
	if (!wpa_s->disconnected &&
	    (!wpa_s->auto_reconnect_disabled ||
	     ...
	     (wpa_s->current_ssid && !wpa_s->current_ssid->no_auto_connect))) { //CONN-WIFI-CD-auto_join_by_ssid_nullpoint-01*
