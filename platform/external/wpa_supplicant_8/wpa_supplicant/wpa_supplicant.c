...
extern void wlan_wapi_iface_receive_pkt_cback(u32 length, u8 *pkt,
                       void *user_data_ptr);
			      ...
			      enum wpa_states state)
{
	...
	wpa_dbg(wpa_s, MSG_INFO, "State: %s -> %s",   //CONN-CD-WIFI-LOG-00*
...
#if !defined(IEEE8021X_EAPOL) || defined(CONFIG_WAPI)
...
#ifdef CONFIG_WAPI
	int sel = 0, proto;
	const u8 *bss_wpa, *bss_rsn, *bss_wapi = NULL;
#else
	int sel, proto;
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (bss) {
		u8*  bss_wapi = wpa_bss_get_ie(bss, WLAN_EID_WAPI);
		wpa_printf(MSG_DEBUG, "%s, %d", __FUNCTION__, __LINE__);
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid) {
		ssid->disabled_for_auto_connect = 0;
		wpas_clear_temp_disabled(wpa_s, ssid, 1);
	}
