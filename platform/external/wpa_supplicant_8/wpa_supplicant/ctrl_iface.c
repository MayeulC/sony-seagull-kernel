...
#ifdef CONFIG_WAPI
	if (!wapi_ie && !ie && !ie2 && bss->caps & IEEE80211_CAP_PRIVACY) {
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strcmp(cmd, "all") == 0) {
		...
		while (ssid) {
			...
			id = ssid->id;
			//ssid may be null in following assignment, and if(!ssid->forbidden_deletion) may make wpa_supplicant crash
			//ssid = ssid->next; //CONN-CD-WIFI-WFD-Certification-00-
			if (!ssid->forbidden_deletion) {
				wpas_notify_network_removed(wpa_s, remove_ssid);
				wpa_config_remove_network(wpa_s->conf, id);
			} else {
				wpa_printf(MSG_ERROR, "CTRL_IFACE: Skip "
				  "removal of read-only network '%s'",
	...
	ssid = wpa_config_get_network(wpa_s->conf, id);
	if (ssid && ssid->forbidden_deletion) {
		wpa_printf(MSG_ERROR, "CTRL_IFACE: Skip removal of "
		  "read-only network '%s'",
		  wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
		return -1;
	}
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (ssid->forbidden_deletion &&
	    ...
	    os_strcmp(name, "no_auto_connect") == 0) {
		wpa_printf(MSG_ERROR, "CTRL_IFACE: Skip modify of "
		  "read-only network '%s'",
		  wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
		return 0;
	}
	...
	else if (os_strcmp(name, "no_auto_connect") == 0)
		ssid->disabled_for_auto_connect = ssid->no_auto_connect;
					      ...
					      size_t buflen)
{
	...
	*name++ = '\0';
	...
	if (value == NULL) {
		wpa_printf(MSG_DEBUG, "CTRL_IFACE: Failed to get cred "
			"variable '%s'", name);
		return -1;
	}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ||
		...
		wpa_hexdump_ascii(level, "RX ctrl_iface",
				  (const u8 *) buf, os_strlen(buf));
		level = MSG_INFO;  //CONN-CD-WIFI-LOG-00+
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		reply_len = wpa_supplicant_ctrl_iface_get_cred(
			wpa_s, buf + 9, reply, reply_size);
#ifndef CONFIG_NO_CONFIG_WRITE
