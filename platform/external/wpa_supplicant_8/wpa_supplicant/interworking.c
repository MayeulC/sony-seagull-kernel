				     ...
				     struct wpa_bss *bss)
{
	...
	ssid->cred_id = cred->id;
	wpa_msg(wpa_s, MSG_INFO, INTERWORKING_SSID_ADDED);
	...
	struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
	...
	if (!wpa_s->imsi[0]) {
		wpa_s->scard = scard_init(SCARD_TRY_BOTH,
					  NULL, wpa_s->apdu_socket);
		if (wpa_s->scard) {
			size_t len = sizeof(wpa_s->imsi) - 1;
			if (scard_get_imsi(wpa_s->scard, wpa_s->imsi, &len)) {
				wpa_printf(MSG_DEBUG, "SCARD: Could not read "
					   "IMSI");
			} else {
				wpa_s->imsi[len] = '\0';
				wpa_s->mnc_len =
					scard_get_mnc_len(wpa_s->scard);
				wpa_printf(MSG_DEBUG, "SCARD: IMSI %s (MNC "
					   "length %d)", wpa_s->imsi,
					   wpa_s->mnc_len);
			}
...
#endif /* PCSC_FUNCS */
#if defined(CONFIG_EAP_PROXY) || defined(PCSC_FUNCS_SEMC)
		if (cred->pcsc && wpa_s->mnc_len > 0 && wpa_s->imsi[0]) {
			imsi = wpa_s->imsi;
			mnc_len = wpa_s->mnc_len;
			goto compare;
		}
#endif /* CONFIG_EAP_PROXY || PCSC_FUNCS_SEMC */
...
#if defined(PCSC_FUNCS) || defined(CONFIG_EAP_PROXY) || defined(PCSC_FUNCS_SEMC)
	compare:
#endif /* PCSC_FUNCS || CONFIG_EAP_PROXY || PCSC_FUNCS_SEMC */
