...
{
	if (ssid->wapi_psk)
		return wpa_config_write_string_ascii((const u8 *)
			ssid->wapi_psk, os_strlen(ssid->wapi_psk));
...
static const struct parse_data ssid_fields[] = {
	...
	{ INT_RANGE(forbidden_deletion, 0, 1) },
	{ INT_RANGE(no_auto_connect, 0, 1) },
	{ STR(id_str) },
#ifdef CONFIG_IEEE80211W
			...
			const char *value, int line)
{
	...
	if (os_strcmp(var, "profile_name") == 0) {
		os_free(cred->profile_name);
		cred->profile_name = val;
		return 0;
	}
...
char * wpa_config_get_cred_no_key(struct wpa_cred *cred, const char *var)
{
	...
	if (os_strcmp(var, "priority") == 0) {
		...
		if (buf != NULL) {
			res = os_snprintf(buf, buflen, "%d", cred->priority);
			if (res < 0 || (unsigned int) res >= buflen) {
				os_free(buf);
				buf = NULL;
			}
		}
	} else if (os_strcmp(var, "pcsc") == 0) {
		buflen = 20;
		buf = os_malloc(buflen);
		if (buf != NULL) {
			res = os_snprintf(buf, buflen, "%d", cred->pcsc);
			if (res < 0 || (unsigned int) res >= buflen) {
				os_free(buf);
				buf = NULL;
			}
		}
	} else if (os_strcmp(var, "eap") == 0) {
		if (cred->eap_method) {
			const char *name;
	...
	} else if (os_strcmp(var, "realm") == 0) {
		if (cred->realm) {
			buf = os_strdup(cred->realm);
		}
	} else if (os_strcmp(var, "username") == 0) {
		if (cred->username) {
			buf = os_strdup(cred->username);
		}
	} else if (os_strcmp(var, "password") == 0) {
		if (cred->password && cred->password[0]) {
			buf = os_strdup("*");
		}
	} else if (os_strcmp(var, "ca_cert") == 0) {
		if (cred->ca_cert) {
			buf = os_strdup(cred->ca_cert);
		}
	} else if (os_strcmp(var, "client_cert") == 0) {
		if (cred->client_cert) {
			buf = os_strdup(cred->client_cert);
		}
	} else if (os_strcmp(var, "private_key") == 0) {
		if (cred->private_key) {
			buf = os_strdup(cred->private_key);
		}
	} else if (os_strcmp(var, "private_key_passwd") == 0) {
		if (cred->private_key_passwd && cred->private_key_passwd[0]) {
			buf = os_strdup("*");
		}
	} else if (os_strcmp(var, "imsi") == 0) {
		if (cred->imsi) {
			buf = os_strdup(cred->imsi);
		}
	} else if (os_strcmp(var, "milenage") == 0) {
		if (cred->milenage) {
			buf = os_strdup(cred->milenage);
		}
	} else if (os_strcmp(var, "domain") == 0) {
		if (cred->domain) {
			buf = os_strdup(cred->domain);
		}
	} else if (os_strcmp(var, "phase1") == 0) {
		if (cred->phase1) {
			buf = os_strdup(cred->phase1);
		}
	} else if (os_strcmp(var, "phase2") == 0) {
		if (cred->phase2) {
			buf = os_strdup(cred->phase2);
		}
	} else if (os_strcmp(var, "roaming_consortium") == 0) {
		if (cred->roaming_consortium_len > 0) {
			...
			if (buf != NULL) {
				for (i = 0; i < cred->roaming_consortium_len; i++) {
					/* wpa_supplicant stores roaming consortium parameter
					 * with byte string(e.g. 001bc504bd). It is converted
					 * to string here.
					 */
					res = os_snprintf(&buf[i * 2], 3, "%02x",
						 cred->roaming_consortium[i]);
					if (res < 0 || (unsigned int) res >= 3) {
						os_free(buf);
						buf = NULL;
						break;
					}
	...
	} else if (os_strcmp(var, "excluded_ssid") == 0) {
		if (cred->num_excluded_ssid > 0) {
			int i;
			buflen = 1;
			...
			if (buf != NULL) {
				...
				for (i = 0; i < cred->num_excluded_ssid; i++) {
					struct excluded_ssid *e = &cred->excluded_ssid[i];
					*pos++ = '\"';
					os_memcpy(pos, e->ssid, e->ssid_len);
					pos += e->ssid_len;
					*pos++ = '\"';
					*pos++ = '\n';
				}
	...
	} else if (os_strcmp(var, "profile_name") == 0) {
		if (cred->profile_name) {
			buf = os_strdup(cred->profile_name);
		}
	}
