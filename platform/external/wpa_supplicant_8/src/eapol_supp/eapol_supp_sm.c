...
static void eapol_sm_txSuppRsp(struct eapol_sm *sm)
{
	...
	if (sm->use_eap_proxy) {
		...
		if (proxy_resp == NULL) {
			wpa_printf(MSG_ERROR, "EAPOL: txSuppRsp - EAP Proxy "
...
{
#ifdef CONFIG_EAP_PROXY   //CONN-FY-Enable_USIM/SIM_simulator++
	char temp_imsi[20];
	size_t temp_imsi_len = 0;
#endif                    //CONN-FY-Enable_USIM/SIM_simulator--
	...
	sm->config = config;
#ifdef CONFIG_EAP_PROXY
    eap_proxy_get_imsi(sm->eap_proxy, temp_imsi, &temp_imsi_len);
	if ( config && eap_allowed_method(sm->eap,
                       EAP_VENDOR_IETF , EAP_TYPE_SIM) 
                &&  temp_imsi_len > 0) {  //CONN-FY-Enable_USIM/SIM_simulator
               if (NULL != sm->eap_proxy)
                       sm->eap_proxy->eap_type =  EAP_TYPE_SIM;
               sm->use_eap_proxy = TRUE;
               ...
               wpa_printf(MSG_DEBUG, "EAPOL: eap proxy disabled");  //CONN-FY-Enable_USIM/SIM_simulator
