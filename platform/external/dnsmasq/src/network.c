...
//CONN-EC-WIFI-NickiSS_JBMR2-00957-01-            die(_("unknown interface given %s in set_interfaces: %s"), if_tmp->name, EC_BADNET);
//CONN-EC-WIFI-NickiSS_JBMR2-00957-01+[
            /* Previously we used to die when set_interfaces was called on an interface
            which was not UP. This lead to an issue when an untether request was called instantly
            after the tether command, hence dnsmasq from tether request couldn't find the
            interface as the untether command had already removed the interface. Hence
            we are allowing the dnsmasq process to run because it will be terminated part of the
            untether process.*/
