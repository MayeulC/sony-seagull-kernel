 ...
 * Copyright (C) 2013 Sony Mobile Communcations AB
 *
...
void WMLImageElement::parseMappedAttribute(Attribute* attr)
{
    ...
    if (attrName == HTMLNames::altAttr) {
        ...
        // WML uses HTMLElement::addHTMLAlignmentToStyledElement to parse align attribute,
        // but for "middle" attr, value CSSValueWebkitBaselineMiddle makes this looks incorrect,
        // so use value CSSValueMiddle to correct align middle attr.
        // Same with align "center" in HTML.
        if (equalIgnoringCase(attr->value(), "middle")) {
            addCSSProperty(attr, CSSPropertyVerticalAlign, CSSValueMiddle);
        } else {
