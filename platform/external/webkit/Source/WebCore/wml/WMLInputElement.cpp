...
void WMLInputElement::defaultEventHandler(Event* evt)
{
    ...
    if (evt->type() == eventNames().textInputEvent && evt->isTextEvent()) {
        ...
        if (textEvent->data() == "\n")
            clickDefaultFormButton = true;
                ...
                && !isConformedToInputMask(textEvent->data()[0],
                    toRenderTextControl(renderer())->text().length() + 1)) {
    ...
    if (evt->type() == eventNames().webkitBeforeTextInsertedEvent){
        BeforeTextInsertedEvent* textEvent = static_cast<BeforeTextInsertedEvent*>(evt);
        unsigned selectionLength = focused() ?
            numGraphemeClusters(plainText(document()->frame()->selection()->selection()
                    .toNormalizedRange().get()))
            : 0;
        RefPtr<Range> range = document()->frame()->selection()->selection().toNormalizedRange();
        String candidateString = toRenderTextControlSingleLine(renderer())->text();
        if (selectionLength)
        ...
        if (!isConformedToInputMask(candidateString)) {
            textEvent->setText("");
            return;
        }
    }
    ...
    else if (evt->type() == eventNames().webkitEditableContentChangedEvent
