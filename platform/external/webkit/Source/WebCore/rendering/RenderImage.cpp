...
// FIXME: the img alt implementation is incorrect (not follow the W3C specification) in Webkit,
//        FP13232 need fix this for WML, for minimize the impact and risk, I made this patch
//        only for WML by control this use macro, but the solution works as well for HTML,
//        suggest submit a patch for Webkit in future.
#if ENABLE(WML)
            if (node()->isWMLElement()) {
                if (m_altText != NULL) {
                    String text = document()->displayStringModifiedByEncoding(
                            m_altText);
                ...
                } else if (m_imageResource->errorOccurred() && !image->isNull()
                    ...
                    context->drawImage(image.get(), style()->colorSpace(),
                            IntPoint(tx + imageX, ty + imageY));
                }
...
bool RenderImage::isLogicalHeightSpecified() const
{
    switch (style()->logicalHeight().type()) {
        case Fixed:
            return true;
        case Percent:
            // [SonyEricsson]: According to section 10.6.2 in CSS 2.1, if a replaced element with
            // a percentage height has a containing block with no explicitly specified height,
            // then it should be assumed to have no intrinsic height. However, this does not apply
            // in quirks mode.
            if (document()->inQuirksMode() || isPositioned()) {
                return true;
            }
            else {
                RenderBlock* cb = containingBlock();
                return (cb && !cb->style()->height().isAuto());
            }
